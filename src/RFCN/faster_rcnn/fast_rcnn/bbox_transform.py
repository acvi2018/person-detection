...
def bbox_transform(ex_rois, gt_rois):
    ...
    if (np.min(gt_widths)<0):
        print(np.where(gt_widths<0))
        print(gt_rois[:, 2])
        print(gt_rois[:, 0])
        print(type(gt_rois))
        print(type(gt_rois[:, 2]))
        print(type(gt_widths))
    # Line added for the error. Short term fix 
    #gt_widths = np.where(gt_widths<0, 1, gt_widths)
